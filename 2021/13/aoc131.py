...
def fold_paper(paper, fold):
    ...
    if fold[0] == 'y':
        folded_paper = []
        first_half  = paper[0:fold[1]]
        second_half = paper[fold[1]+1:]
        second_half.reverse()
        folded_paper = [list(map(add, first_half[i], second_half[i])) for i in range(len(first_half))]
    else:
        first_half  = [paper[i][0:fold[1] ] for i in range(len(paper))]
        second_half = [paper[i][fold[1]+1:] for i in range(len(paper))]
...
def main():
    ...
    for line in folded_paper:
        number_of_marks += sum([1 for x in line if x > 0])
