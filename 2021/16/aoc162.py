...
class Root:
    def __init__(self, operator: int, nodes: List[Node]):
        self.nodes = nodes
        if operator == 0:
            # Sum
            ...
            self.operator = lambda x,y: 1 if x > y else 0
        elif operator == 6:
    ...
    def evaluate(self) -> int:
        return reduce(self.operator,[node.evaluate() for node in self.nodes])
...
def parse(bit_string: str) -> Tuple[Node,int]:
    ...
    if bit_string[6] == '0':
        ...
        return Root(type_id,packages),number_of_bits+22
    ...
    if bit_string[6] == '1':
        ...
        return Root(type_id,packages), bits_parsed+18
...
def main():
    ...
    number,string_length = parse(bit_string)
