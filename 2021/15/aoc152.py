...
def create_bigger_cave_map(cave_map):
    """Burde plassere lambda i egen funksjon"""
    new_cave = cave_map
    for _ in range(4):
        new_cave = list(map((lambda line : list(map((lambda x: x+1 if x < 9 else 1),line))),new_cave))
        for i,line in enumerate(new_cave):
            cave_map[i] += line
...
def main():
    ...
    # This is cost "map"    
    cave_map_orginal = [[int(cha) for cha in line.strip()] for line in lines]
    cave_map = create_bigger_cave_map(cave_map_orginal)
    ...
    _, cost_so_far = dijkstra_search(cave, start,goal)
