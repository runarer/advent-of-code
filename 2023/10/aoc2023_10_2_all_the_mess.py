"""Advent of Code: 2019.1.1
    ...
    FROM REDDIT, Hva jeg prøvde å gjøre først.
...
class PipeMap:
    ...
    def get_pipe_line(self):
        #pipe_line = [ [] for _ in self.pipe_map]
        #pipe_line = { self.start }
        pipe_line = [self.start]
        prev_pos = self.start
        #pipe_line[self.start[0]].append(self.start[1])
        ...
        #pipe_line.add(cur_pos)
        #pipe_line.append(cur_pos)
        #pipe_line[cur_pos[0]].append(cur_pos[1])
        ...
        while (s := self.get_tile(cur_pos)) != 'S':
            ...
            if prev_pos != ( t := self.add_tup(cur_pos,self.moves[s][0])):
                ...
                cur_pos = temp
            #pipe_line[cur_pos[0]].append(cur_pos[1])
            #self.pipe_map[prev_pos[0]][prev_pos[1]] = '*'
            #pipe_line.add(cur_pos)
    ...
    def calculate_enclosure(self):
        space = 0
        #pipe_line = [ sorted(l) for l in self.get_pipe_line()]
        pipe_line = self.get_pipe_line()
        coloured_map = [ l.copy() for l in self.pipe_map]
        print(pipe_line)

        for i,_ in enumerate(coloured_map[0]):
            if (0,i) in pipe_line:
                coloured_map[0][i] = 'B'
            else:
                coloured_map[0][i] = '0'
        ...
        for i, line in enumerate(coloured_map[1:],start=1):
            for j,tile in enumerate(line):
                if (i,j) in pipe_line:
                    if coloured_map[i-1][j] == '0' or coloured_map[i-1][j] == 'E':
                        coloured_map[i][j] = 'B'
                    elif coloured_map[i-1][j] == "I":
                        coloured_map[i][j] = 'E'
                    elif coloured_map[i-1][j] == "B":
                        if tile in "-LJ":
                            coloured_map[i][j] = 'E'
                        else:
                            coloured_map[i][j] = 'B'
                    ...
                    if coloured_map[i-1][j] == '0' or coloured_map[i-1][j] == 'E':
                        coloured_map[i][j] = '0'
                    if coloured_map[i-1][j] == 'I' or coloured_map[i-1][j] == 'B':
                        coloured_map[i][j] = 'I'


        for line in coloured_map:
            print(line)
        
        return space

    def new_calculate_enclosure(self):
        pipe_line = self.get_pipe_line()
        #bigger_map = { self.get_tile(p) for p in pipe_line }
        bigger_map = set()

        for pos in pipe_line:
            for n in self.big_tile[self.get_tile(pos)]:
                bigger_map.add( (pos[0]*3+n[0],pos[1]*3+n[1]) )
...
def main():
    ...
    pm = PipeMap(pipe_map)
    pm.walk_the_pipe()
    #f = pm.calculate_enclosure()
    #pm.new_calculate_enclosure()
    
    #print(pm.get_pipe_line())
...
".......││┌──┘│└─┘│└┐││┌┘│.└─┐┌┘┌┘└─┘┌┐└┘└┘└─┐┌─┘└─┘│││┌┘└┐┌┐└───┐┌──┘┌┘└─┐││┌┐│I┘└─┘│└┐┌┘└┐┌┘└─┘└┘└┘┌──┘└──────┐┌┘└┘│└┐.└┐└┘││┌───┘......... 52"
".......||F--J|L-J|L7||FJ|.L-7FJFJL-JF7LJLJL-7F-JL-J|||FJL7F7L---7F--JFJL-7||F7|FJL-J|L7FJL7FJL-JLJLJF--JL------7FJLJ|L7.L7LJ||F---J......... 52"