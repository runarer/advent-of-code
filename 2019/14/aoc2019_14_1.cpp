...
int oreNeed(ReactionMap const& reactions, const int amount, const std::string name) {
    ...
    while(!needed.empty()) {
        /* Grab first element */
        int amountNeeded{ needed.front().second };
        ...
        int producedPerRound{ reactions.at(toProduce).first };

        /* Is there any surplus */
        int surplusOfProduct{ surplus.at(toProduce) };
        ...
        /* Produce needed */
        int roundNeeded{ amountNeeded / producedPerRound };        
...
int oreNeeded(ReactionMap const &reactions, const int amount, const std::string name) {
    if (name == "ORE") return amount;

    int ore{ 0 };
    
    /* Find multiplier, we want amount but reaction produces reaction.at(name).first */
    int reactionRuns{ amount / reactions.at(name).first };
    if (amount % reactions.at(name).first > 0) ++reactionRuns;

    /* Find ore for reagents */
    for (auto reagent : reactions.at(name).second) {
        ore += reactionRuns * oreNeeded(reactions, reagent.first, reagent.second);
    }
...
int main(int argc, char* argv[]) {
    ...
    int ore = oreNeed(reactions, 1, "FUEL");
    std::cout << "Ore Needed: " << ore << "\n";

    /* Print Map */
    /*for (auto reaction : reactions) {
        std::cout << reaction.second.first << " " << reaction.first << " created by";
        for (auto reagent : reaction.second.second) {
            std::cout << " " << reagent.first << " " << reagent.second;
        }
        std::cout << '\n';
    }*/
