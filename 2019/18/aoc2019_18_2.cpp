...
/*
    Legg til ekstra vegger.
    Gjør cave enklere.
    Lag fire kopier av cave med hvert sitt startpunkt.
    Finn key og doors fra start i hver kopi, ignorer dører.
    Fjern dører som ikke har tilsvarende key.
...
CaveMap::CaveMap(const std::vector<std::string>& map) : mapH{ static_cast<int>(map.size()) }, mapW{ static_cast<int>(map[0].size()) }
{
    ...
    cavemap[start.first+1][start.second] = '#';
    cavemap[start.first-1][start.second] = '#';
    cavemap[start.first][start.second+1] = '#';
    cavemap[start.first][start.second-1] = '#';

    /* Add starts */
    cavemap[start.first + 1][start.second + 1] = '@';
    cavemap[start.first - 1][start.second - 1] = '@';
    cavemap[start.first - 1][start.second + 1] = '@';
    cavemap[start.first + 1][start.second - 1] = '@';
    ...
    std::string solutions[]{"iaxjblcr","wqtgkmn", "vszopuh","ydef"}; // Funnet manuelt.
    
    int steps = 34+96+84+46-8; // Fra oppgave 1
