...
std::map<std::pair<int, int>, int> Robot::exploreMap() {
    ...
    move(Direction::north, 0, -1);
    move(Direction::east, 1, 0);
    move(Direction::south, 0, 1);
    move(Direction::west, -1, 0);

    return map;
}

std::vector<std::vector<int>> createMap(std::map<std::pair<int, int>, int> &room) {
    int minX{ 0 };
    int minY{ 0 };
    int maxX{ 0 };
    int maxY{ 0 };
...
std::pair<int, int> findOxygenSystem(std::vector<std::vector<int>>& roomMap) {
    for (int i{ 0 }; i < roomMap.size(); ++i)
        for (int j{ 0 }; j < roomMap[i].size(); ++j)
            if (roomMap[i][j] == 2)
...
int fillOxygen(std::vector<std::vector<int>> &roomMap,Direction direction, int x, int y) {
    if (roomMap[y][x] == 0) return 0;

    std::array ret{ 0,0,0,0 };
    if(direction != Direction::south)
        ret[0] = fillOxygen(roomMap, Direction::north, x, y - 1);
    if (direction != Direction::west)
        ret[1] = fillOxygen(roomMap, Direction::east, x+1, y);
    if (direction != Direction::north)
        ret[2] = fillOxygen(roomMap, Direction::south, x, y + 1);
    if (direction != Direction::east)
        ret[3] = fillOxygen(roomMap, Direction::west, x-1, y);
...
int main(int argc, char* argv[]) {
    ...
    /* Create a map */
    std::vector<std::vector<int>> roomMap{ createMap(room)};
            
    /* Finds 2's position. */
    std::pair<int,int> oxygenSystem{ findOxygenSystem(roomMap)};
    std::cout << "OS: (" << oxygenSystem.first << ","<< oxygenSystem.second << ")\n";

    /* Start at 2 and find longest path to a deadend */
    int minutes{ fillOxygen(roomMap,Direction::north, oxygenSystem.first, oxygenSystem.second-1) };
    std::cout << "It takes " << minutes << " minutes to fill the room.\n";
